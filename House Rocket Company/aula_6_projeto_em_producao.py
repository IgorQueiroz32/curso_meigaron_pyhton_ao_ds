...
def overview_data(data):
    ...
    num_attributes = data.select_dtypes(
        include=['int64', 'float64'])  # seleciona as colunas pelo tipo, neste caso, numericas

    # central tendency - media, mediana
    media = pd.DataFrame(
        num_attributes.apply(np.mean, axis=0))  # funcao apply aplica qualquer funcao nas colunas ou linhas da variaves
